...
export async function GET(req: Request) {
  ...
  const contestantIds = [...new Set((members || []).map((member: any) => member.contestant_id))];
  ...
  const contestantsById = new Map((contestants || []).map((contestant: any) => [contestant.id, contestant]));
